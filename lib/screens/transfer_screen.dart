...
class _TransferScreenState extends State<TransferScreen>
  ...
  Future<void> _loadRecipientNames() async {
    ...
    for (final t in toLoad) {
      try {
        final result = await authRepo.lookupAccountByNumber(
          (await authRepo.getWalletByUserId(t.receiverId))?.accountNumber ?? '',
        );
  ...
  Future<void> _selectRecentTransfer(TransactionModel t) async {
    ...
    try {
      final authRepo = context.read<AuthRepository>();
      final wallet = await authRepo.getWalletByUserId(t.receiverId);
      if (wallet == null) {
      ...
      final result = await authRepo.lookupAccountByNumber(wallet.accountNumber);
      final user = result['user'] as UserModel;
      if (mounted) {
        setState(() {
          _recipientUser = user;
  ...
  Future<void> _lookupAccount() async {
    ...
    try {
      final result = await context.read<AuthRepository>().lookupAccountByNumber(
        number,
      );
      final user = result['user'] as UserModel;
      if (mounted) {
          ...
          if (recentList.isNotEmpty)
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                Divider(height: 1, color: theme.primaryMid.withValues(alpha: 0.25)),
                  Padding(
                ...
                children: [
                  AnimatedContainer(
                    ...
                    child: Icon(
                      cat.icon,
                      size: 20,
                      color: isSel ? Colors.white : NovaTheme.of(context).primary,
                    ),
                  ),
    ...
    child: Row(
      children: [
        ...
        Switch(
          ...
          activeTrackColor: NovaTheme.of(context).primary.withValues(alpha: 0.4),
        ),
      ],
    ),
  );
