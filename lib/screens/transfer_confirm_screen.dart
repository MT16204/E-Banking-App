  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(context.tr('Xác nhận giao dịch', 'Confirm transaction'),
            style: NovaFonts.heading.copyWith(fontSize: 18, color: theme.textPrimary)),
        actions: [
          IconButton(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: theme.primaryMid.withValues(alpha: 0.35)),
              ),
              child: Row(
                children: [
                  Container(
            ...
            Container(
              ...
              child: Column(
                children: [
                  _row(context, context.tr('Tài khoản nguồn', 'Source account'), sourceAccountNumber),
                  _divider(context),
                  _row(context, context.tr('Tài khoản thụ hưởng', 'Recipient account'), recipientAccountNumber,
                      valueColor: NovaColors.yellow),
                  _divider(context),
                  _row(context, context.tr('Tên người thụ hưởng', 'Recipient name'), recipientName,
                      valueColor: NovaColors.yellow),
                  _divider(context),
                  _row(context, context.tr('Ngân hàng thụ hưởng', 'Recipient bank'), recipientBank),
                  _divider(context),
                  _row(
                    context,
                  ...
                  _divider(context),
                  _row(context, context.tr('Phí giao dịch', 'Transaction fee'), context.tr('Miễn phí', 'Free')),
                  _divider(context),
                  _row(context, context.tr('Số tiền trích nợ', 'Debited amount'), '${fmt.format(amount)} VND',
                      valueColor: NovaColors.yellow,
                      valueFontSize: 16,
                      valueBold: true),
                  _divider(context),
                  _row(context, context.tr('Ngày giao dịch', 'Transaction date'), dateStr),
                  _divider(context),
                  _row(context, context.tr('Nội dung', 'Description'), note, multiLine: true),
                ],
              ),
            ),
          ],
      ...
      bottomNavigationBar: Container(
        ...
        child: SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: Text(context.tr('Xác nhận', 'Confirm'),
                style: NovaFonts.heading
      ...
      child: Row(
        ...
        children: [
          Expanded(
            flex: 2,
            child: Text(label,
                style: NovaFonts.body
                    .copyWith(color: NovaTheme.of(context).textSecondary, fontSize: 14)),
          ),
...
class _OtpBottomSheetState extends State<OtpBottomSheet> {
  ...
  Future<void> _executeTransfer() async {
    ...
    _transferDone = true;
    try {
      await context.read<AuthRepository>().transferMoney(
        ...
        child: Column(
          ...
          children: [
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(context.tr('Xác nhận giao dịch', 'Confirm transaction'),
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      width: 36, height: 36,
                      decoration: BoxDecoration(
                          color: theme.background, shape: BoxShape.circle),
            ...
            if (_hasError) ...[
              ...
              Text(context.tr('Mã PIN không đúng. Vui lòng thử lại.', 'Incorrect PIN. Please try again.'),
                  style: NovaFonts.body.copyWith(
    ...
    children: keys.map((key) {
      return Expanded(
        child: GestureDetector(
          onTap: key.isEmpty ? null : () => _onKeyTap(key == 'del' ? 'del' : key[0]),
          child: Container(
            height: 72,
            ...
            child: Center(
              child: key == 'del'
                  ? Icon(Icons.backspace_outlined,
                      size: 22, color: NovaTheme.of(context).textPrimary)
                  : key.isEmpty
                      ? const SizedBox()
                      : Column(
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 20),
                      _summaryRow(context.tr('Người nhận', 'Recipient'), recipientName,
                          valueColor: theme.primary),
                      Divider(height: 24, color: theme.primaryMid.withValues(alpha: 0.25)),
                      _summaryRow(context.tr('Số tài khoản', 'Account number'), recipientAccountNumber),
                      Divider(height: 24, color: theme.primaryMid.withValues(alpha: 0.25)),
                      _summaryRow(context.tr('Nội dung', 'Description'), note),
                      Divider(height: 24, color: theme.primaryMid.withValues(alpha: 0.25)),
                      _summaryRow(context.tr('Phí giao dịch', 'Transaction fee'), context.tr('Miễn phí', 'Free')),
                    ],
                  ),
                ),
